...
exports.ApiError = exports.CODE_STATUS = void 0;
...
function problemDetails(request, code, detail) {
    return {
        type: `${BASE_URI}/${code.toLowerCase()}`,
        title: CODE_DETAIL[code],
        status: exports.CODE_STATUS[code],
        detail: detail ?? CODE_DETAIL[code],
        instance: request.url ?? "/",
        requestId: request.id,
    };
}
function sendError(reply, code, detail) {
    const status = exports.CODE_STATUS[code];
    const body = problemDetails(reply.request, code, detail);
    reply.status(status).send(body);
}
function throwError(code, detail) {
    const err = new Error(detail ?? CODE_DETAIL[code]);
    err.code = code;
    throw err;
}
